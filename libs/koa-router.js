/*
 * @Author: Yorn Qiu
 * @Date: 2022-04-12 15:19:14
 * @LastEditors: Yorn Qiu
 * @LastEditTime: 2024-01-31 16:30:24
 * @FilePath: /node-cas/libs/koa-router.js
 * @Description: koa router
 */

/**
 * RESTful router middleware for koa
 *
 * method: 'GET'
 * path: '/user'
 * route: 'GET /user'
...
class Router {
   ...
   * @returns
   */
  registerRoute(method, path, routeHandler, options) {
    // set prefix
    if (options.prefix) {
   ...
   * @returns
   */
  registerMiddleware(path = '', middlewareHandler, options) {
    // set prefix
    if (options.prefix) {
      path = join(options.prefix, path)
    }
   ...
   * @param {Router} router
   */
  registerNestedRouter(path, router) {
...
/**
 * execute router handler as order
 * @param {Array} handlers
 */
async function compose(handlers, context) {
  try {
    const fn = handlers.shift()
    if (!fn) return Promise.resolve()

    await fn(context)
    await compose(handlers, context)
  } catch (err) {
